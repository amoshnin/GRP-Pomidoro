...
const PrivateCabinetScreen: React.FC<PropsType> = (props) => {
  ...
  return (
    <Stack.Navigator initialRouteName="PrivateCabinetScreenMain">
        ...
        options={({ navigation, route }: any) => ({
          header: () => (
            <GeneralHeader
              ...
              leftIcon={<MaterialIcons name="menu" size={24} color="#1C1C1C" />}
              pressLeftIcon={() => navigation.navigate("DashboardScreen")}
            />
          ),
        })}
      />
        ...
        options={({ navigation, route }: any) => ({
          header: () => (
            <GeneralHeader
              navigation={navigation}
              leftIcon={
                <Ionicons name="ios-arrow-back" size={24} color="#1C1C1C" />
              }
              pressLeftIcon={() => navigation.goBack()}
            />
          ),
        })}
      />
      ...
      <Stack.Screen
        name="PaymentScreen"
        component={PaymentScreenContainer}
        options={({ navigation, route }: any) => ({
          header: () => (
            <GeneralHeader
              navigation={navigation}
